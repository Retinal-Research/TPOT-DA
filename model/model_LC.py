...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size, bias): ## default n_feat = 80
        super(SAM, self).__init__()
        self.conv1 = spectral_norm(conv(n_feat, n_feat, kernel_size, bias=bias))
        self.conv2 = spectral_norm(conv(n_feat, 3, kernel_size, bias=bias))
        self.conv3 = spectral_norm(conv(3, n_feat, kernel_size, bias=bias))
    ...
    def forward(self, x, x_img):
        ...
        x1 = x1*x2
        x1 = x1+x
        # img = torch.clamp(img,min=0.0,max=1.0)  #### during domain adaptation, just erase clamp ###
        return x1, img
...
class EfficientCALayer(nn.Module):
    def __init__(self, k_size = 3):
        ...
        self.conv = spectral_norm(nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1)//2, bias=True))
...
class CAB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        modules_body = []
        modules_body.append(spectral_norm(conv(n_feat, n_feat, kernel_size, bias=bias)))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        ...
        enc3 = self.encoder_level3(x) ## default channel = 176
        if (encoder_outs is not None) and (decoder_outs is not None):
...
class DownSample(nn.Module):
    def __init__(self, in_channels,s_factor):
        ...
        self.down = nn.Conv2d(in_channels, in_channels + s_factor, kernel_size=3, stride=2, padding=1, bias=False)
...
class _NetG(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=80, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=[3,5,7], bias=False):
        ...
        act=nn.PReLU()
        self.shallow_feat1 = nn.Sequential(spectral_norm(conv(3, n_feat, kernel_size, bias=bias)), CAB(n_feat,kernel_size, 3, bias=bias, act=act))
    ...
    def forward(self, x3_img):
        ...
        return stage1_img ## right now it's not in the sigmoid range
